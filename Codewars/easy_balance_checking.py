...
def balance(book):
    org_balance = float(re.match(re.compile(r"^\d*\.?\d*"), book)[0])
    cur_balance = org_balance
    rep = "Original Balance: {:.2f}\r\n".format(org_balance)
    shopping_list = re.findall(re.compile(r"(\d{3})\s([A-Z]\w*)\D*(\d*\.?\d*)"), book)
