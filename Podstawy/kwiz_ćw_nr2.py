...
def show_highest_scores():
    highest_scores = []
    all_scores = open('kwiz_scores.txt', 'r')
    name = all_scores.readline()
    name = name[:-1]
    score = all_scores.readline()
    score = score[:-1]
    score = int(score)
    while name != '':
        entry = (name, score)
        highest_scores.append(entry)
        name = all_scores.readline()
        name = name[:-1]
        score = all_scores.readline()
        score = score[:-1]
        try:
            ...
            pass
    all_scores.close()
    highest_scores.sort(key=lambda tup: tup[1], reverse=True)
...
def main():
    ...
    all_scores = open('kwiz_scores.txt', 'a')
    all_scores.write('{}\n'.format(name))
    all_scores.write('{}\n'.format(score))
    all_scores.close()
    show_highest_scores()
