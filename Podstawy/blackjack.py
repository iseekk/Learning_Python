...
class BJGame(object):
    ...
    def play(self):
        ...
        # obstawianie
        for player in self.players:
            self.player_bet = player.is_betting()
            player.money -= self.player_bet
            self.bank += self.player_bet
        ...
        if not self.still_playing:
            ...
            amount_to_give = self.bank // len(self.still_playing)
            self.bank = self.bank - (self.bank % len(self.still_playing))

            if self.dealer.is_busted():
                # wygrywa każdy, kto jeszcze pozostaje w grze
                for player in self.still_playing:
                    player.money += amount_to_give
                    player.win()
            else:
